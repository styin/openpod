//! Verified QUIC connection wrapper.
//!
...
impl PodConnection {
    ...
    pub fn export_keying_material(&self) -> Result<Vec<u8>> {
        ...
        self.inner
            ...
            .map_err(|e| AgentError::PeerIdentity(format!("keying material export: {e:?}")))?;
        Ok(output)
    }
    ...
    pub fn send_control_datagram(&self, signal: &ControlSignal) -> Result<()> {
        ...
        self.inner
            ...
            .map_err(|e| AgentError::Datagram(format!("send control: {e}")))?;
        Ok(())
    }
    ...
    pub fn send_audio_datagram(&self, frame: &AudioFrame) -> Result<()> {
        ...
        self.inner
            .send_datagram(Bytes::from(tagged))
            .map_err(|e| AgentError::Datagram(format!("send audio: {e}")))?;
        Ok(())
    }
...
fn extract_peer_pod_id(conn: &quinn::Connection) -> Result<PodId> {
    let identity = conn
        .peer_identity()
        .ok_or_else(|| AgentError::PeerIdentity("no peer identity available".into()))?;

    let certs = identity
        .downcast::<Vec<CertificateDer<'static>>>()
        .map_err(|_| AgentError::PeerIdentity("failed to downcast peer identity".into()))?;

    let leaf = certs
        .first()
        .ok_or_else(|| AgentError::PeerIdentity("peer certificate chain is empty".into()))?;

    let pubkey = pod_proto::tls::cert_extract::extract_ed25519_public_key(leaf)
        .map_err(|e| AgentError::PeerIdentity(format!("key extraction: {e}")))?;
