...
pub async fn write_tagged_message<M: Message>(
    ...
    send.write_all(&[tag as u8])
        .await
        .map_err(|e| AgentError::StreamIo(format!("write tag: {e}")))?;
    let buf = msg.encode_to_vec();
    send.write_all(&buf)
        .await
        .map_err(|e| AgentError::StreamIo(format!("write: {e}")))?;
    send.finish()
        .map_err(|e| AgentError::StreamIo(format!("finish: {e}")))?;
    Ok(())
}
...
pub async fn read_stream_tag(recv: &mut RecvStream) -> Result<StreamTag> {
    ...
    recv.read_exact(&mut tag_buf)
        .await
        .map_err(|e| AgentError::StreamIo(format!("read tag: {e}")))?;
    StreamTag::from_byte(tag_buf[0])
        .ok_or_else(|| AgentError::StreamIo(format!("unknown stream tag: 0x{:02x}", tag_buf[0])))
}
...
pub async fn read_message<M: Message + Default>(recv: &mut RecvStream) -> Result<M> {
    let buf = recv
        ...
        .await
        .map_err(|e| AgentError::StreamIo(format!("read: {e}")))?;

    M::decode(buf.as_slice()).map_err(|e| AgentError::StreamIo(format!("decode: {e}")))
}
