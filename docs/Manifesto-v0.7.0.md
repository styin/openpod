...
* **The SDK (`pod-sdk`):** A lightweight adapter library imported into the Agent Gateway. *It does not invoke LLMs.* It exposes a QUIC server, decrypts the incoming Pod connection, extracts the semantic JSON payload, and hands it directly to the Gateway via callbacks/events.
...
* **Transport:** **QUIC (RFC 9000)** via `quinn`. This natively supports both *Reliable Streams* (for Tokens/Logs) and *Unreliable Datagrams* (for immediate Control Interrupts). Migration path: WebTransport (HTTP/3 framing) can be layered on top when browser clients are needed and the `wtransport` ecosystem matures.
