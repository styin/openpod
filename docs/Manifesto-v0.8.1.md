...
**v0.8.1 — Stream-Type Tag Multiplexing**

- **Added 1-byte stream-type tags for all post-session-init QUIC streams.** After session establishment (§2.11.1), every new bidirectional stream carries a 1-byte tag prefix identifying its purpose. This is the reliable-stream analog of the datagram channel tags introduced in v0.8.0 for Channels C/D. Without stream tags, the receiver cannot distinguish a Channel A envelope from a session close request when calling `accept_bi()`, because protobuf3 silently decodes any bytes as any message type (unknown fields are skipped, missing fields get zero defaults). This made stream misrouting undetectable and blocked future features (file uploads §2.6, Channel B telemetry) that open additional stream types on the same connection.
- **Stream tags are symmetric to datagram tags** but occupy a separate namespace — streams and datagrams are different QUIC primitives and never need cross-disambiguation. The tag byte is written first so the receiver can dispatch before reading the protobuf body.
- **Handshake and session-init streams remain untagged.** These are sequential one-shot exchanges at connection startup where both sides know exactly what message to expect. Tags are only needed for the multiplexed post-session-init phase.
- **`SessionClose` is sent on a dedicated tagged stream, not Channel A.** Previous versions described `SessionClose` as a Channel A message (§2.11.2). In practice, close is a connection-level operation that must not compete with Channel A's application-layer sequencing. It now travels on its own tagged stream (`0x02`) and is dispatched by the receiver's stream-type tag router alongside Channel A envelopes (`0x01`).
...
**Stream-type tag multiplexing:** After session establishment (§2.11.1), all QUIC bidirectional streams carry a 1-byte stream-type tag prefix as their first byte. This enables a single `accept_bi()` loop to dispatch incoming streams by type — the receiver reads the tag, then decodes the appropriate protobuf message. Stream tags occupy a separate namespace from datagram tags (streams and datagrams are different QUIC primitives).

| Tag    | Stream Type       | Payload                          |
| ------ | ----------------- | -------------------------------- |
| `0x01` | Channel A Envelope | `ChannelAEnvelope` protobuf     |
| `0x02` | Session Close      | `SessionClose` protobuf         |

Unknown stream tags are treated as protocol errors (§2.10, code range `4xxx`). Handshake and session-init streams (§2.9, §2.11.1) are untagged — they are sequential one-shot exchanges at connection startup where both sides know the expected message type.
...
After version negotiation (§2.9), the client sends a `SessionInit` message. The agent responds with a `SessionAck` containing the assigned `session_id`. All subsequent messages are scoped to this session. From this point forward, every new bidirectional stream carries a 1-byte stream-type tag prefix (§2.3) so the receiver can dispatch without ambiguity.

#### 2.11.2 Graceful Shutdown

Either side may initiate a graceful shutdown:

1. The initiator opens a new bidirectional stream with the `Close` stream-type tag (`0x02`, §2.3) and writes a `SessionClose` message with a reason code.
2. The receiver reads the tag, decodes `SessionClose`, and writes `SessionCloseAck` on the same stream's send half.
